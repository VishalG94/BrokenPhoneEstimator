...
const upload = require('../../../kafka-backend/api/config/multer')

router.post('/restaurantmenu', function (req, res) {
  console.log('Inside restaurant menu Post Request')
  console.log(req.body)
  kafka.make_request('restaurant_menu', req.body, function (err, results) {
    // console.log(results);
    if (err) {
      res.writeHead(400, {
...
router.get('/restaurants', function (req, res) {
  ...
  kafka.make_request('restaurants', req.query, function (err, results) {
    // console.log(results);
    if (err) {
      res.writeHead(400, {
        'Content-Type': 'text/plain'
...
router.get('/restaurantsections', function (req, res) {
  ...
  kafka.make_request('restaurant_sections', req.query, function (err, results) {
    // console.log(results);
    if (err) {
      res.writeHead(400, {
        'Content-Type': 'text/plain'
...
router.get('/sectionsmenu', function (req, res) {
  ...
  let temp = req.query.restaurant_name
  kafka.make_request('sections_menu', req.query, function (err, results) {
    // console.log(results);
    if (err) {
      res.writeHead(400, {
        'Content-Type': 'text/plain'
...
router.post('/updaterestaurantmenu', function (req, res) {
  ...
  kafka.make_request('update_restaurant_menu', req.body, function (
    err,
    results
  ) {
    // console.log(results);
    if (err) {
      ...
      res.end('Successfully Updated menu details')
    }
  })
  // console.log("Req Body : ", username + "password : ",password);
  // var now = new Date()
...
router.get('/menudetails', function (req, res) {
  ...
  kafka.make_request('menu_details', req.query, function (err, results) {
    // console.log(results);
    if (err) {
      res.writeHead(400, {
        'Content-Type': 'text/plain'
...
router.post('/menuimage', function (req, res) {
  ...
  kafka.make_request('menu_image', req.body, function (err, results) {
    // console.log(results);
    if (err) {
      res.writeHead(400, {
        'Content-Type': 'text/plain'
      ...
      console.log('response image'+results)
      res.writeHead(200, {
...
router.post('/menuimageform', function (req, res) {
  ...
  upload(req, res, err => {
      if (err) {
        ...
        console.log('Inside upload menu image post call')
        console.log(req.file.originalname)
        // console.log(req.file.email);
        if(req.file){
          kafka.make_request('menu_image_form', req.file, function (err, results) {
            // console.log(results);
            if (err) {
              res.writeHead(400, {
                'Content-Type': 'text/plain'
